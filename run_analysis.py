import haloutils
from caterpillaranalysis import *
from subradplugin import *
from caterpillarplot import *
from tbtfplugin import *
import pylab as plt
from optparse import OptionParser

if __name__=="__main__":
    parser = OptionParser()
    parser.add_option("--recalc",action="store_true",dest='recalc',default=False)
    parser.add_option("--eps",action="store_true",dest='eps',default=False)
    parser.add_option("--pdf",action="store_true",dest='pdf',default=False)
    parser.add_option("--sheet",action="store",type=int,dest='sheet',default=1)
    (options,args) = parser.parse_args()
    firsttwelve = get_haloidlist(1)
    ### Use this section for quick testing scripts 
    if len(args) == 0:
        #plug = SubhaloRadialSubmassFracPlugin()
        #convergeplot(1,plug,figfilename='subradsubmassfracLX_s1.png',stop_on_error=True)
        arg = None
    else:
        assert len(args)==1
        arg = int(args[0])

    ### Use this section for essentially finalized analysis plots
    sheet = options.sheet; ext = '_s'+str(sheet)
    myrecalc = options.recalc
    if options.eps: ext+='.eps'
    elif options.pdf: ext+='.pdf'
    else: ext+='.png'
    if arg == 1:
        Nvmax      = NvmaxPlugin()
        convergeplot(sheet,Nvmax,lw=2,figfilename='NvmaxLX'+ext,recalc=myrecalc)
        convergeplot(sheet,Nvmax,lw=2,figfilename='normNvmaxLX'+ext,normtohost=True)
        paper_stackplot(14,Nvmax,figfilename='stackLX14_normNvmax'+ext,lw=2,normtohost=True)
    if arg == 2:
        SHMF       = SHMFPlugin()
        intSHMF    = IntegrableSHMFPlugin()
        convergeplot(sheet,SHMF,lw=2,figfilename='SHMFLX'+ext,recalc=myrecalc)
        convergeplot(sheet,SHMF,lw=2,figfilename='normSHMFLX'+ext,normtohost=True)
        convergeplot(sheet,intSHMF,lw=2,figfilename='intSHMFLX'+ext)
        paper_stackplot(14,SHMF,figfilename='stackLX14_normSHMF'+ext,lw=2,normtohost=True)
    if arg == 3:
        Profile    = ProfilePlugin()
        VelProfile = VelocityProfilePlugin()
        convergeplot(sheet,Profile,figfilename='rhor2LX'+ext,recalc=myrecalc)
        convergeplot(sheet,VelProfile,figfilename='velprofLX'+ext)
        convergeplot(sheet,VelProfile,figfilename='normvelprofLX'+ext,normtohost=True)
        paper_stackplot(14,VelProfile,figfilename='stackLX14_normvelprof'+ext,normtohost=True)
    if arg == 4:
        MassAccr   = MassAccrPlugin()
        LinMassAccr= LinearMassAccrPlugin()
        convergeplot(sheet,MassAccr,figfilename='massaccrLX'+ext,recalc=myrecalc)
        convergeplot(sheet,MassAccr,figfilename='normmassaccrLX'+ext,normtohost=True)
        paper_stackplot(14,MassAccr,figfilename='stackLX14_massaccr'+ext,lw=2)
        paper_stackplot(14,MassAccr,figfilename='stackLX14_normmassaccr'+ext,lw=2,normtohost=True)
        convergeplot(sheet,LinMassAccr,figfilename='linmassaccrLX'+ext)
        convergeplot(sheet,LinMassAccr,figfilename='normlinmassaccrLX'+ext,normtohost=True)
        paper_stackplot(14,LinMassAccr,figfilename='stackLX14_linmassaccr'+ext,lw=2)
        paper_stackplot(14,LinMassAccr,figfilename='stackLX14_normlinmassaccr'+ext,lw=2,normtohost=True)
    if arg == 5:
        SubPhase   = SubPhaseContourPlugin()
        convergeplot(sheet,SubPhase,whichlx=[13,14],figfilename='subphaseLX'+ext,recalc=myrecalc)
    if arg == 6:
        SubProfile     = SubProfilePlugin()
        SubVelProfile  = SubVelocityProfilePlugin()
        TBTF = TBTFPlugin()
        for lx in [12,13,14]:
            convergeplot(sheet,SubProfile,whichlx=[lx],figfilename='subprofLX'+str(lx)+''+ext,recalc=myrecalc)
            convergeplot(sheet,SubVelProfile,whichlx=[lx],figfilename='subvelprofLX'+str(lx)+''+ext)
            convergeplot(sheet,SubVelProfile,whichlx=[lx],figfilename='normsubvelprofLX'+str(lx)+''+ext,normtohost=True)
        convergeplot(sheet,TBTF,whichlx=[14],figfilename='tbtfLX'+str(14)+''+ext)

    if arg == 7:
        SubRad         = SubhaloRadialPlugin()
        intSubRad      = IntegrableSubhaloRadialPlugin()
        SubRadByMass   = SubhaloRadialByMassPlugin()
        intSubRadByMass= IntegrableSubhaloRadialByMassPlugin()
        convergeplot(sheet,SubRad,figfilename='subradLX'+ext,recalc=myrecalc)
        convergeplot(sheet,intSubRad,whichlx=[14],figfilename='intsubradLX14'+ext)
        convergeplot(sheet,SubRadByMass,whichlx=[14],figfilename='subradbymassLX14'+ext)
        convergeplot(sheet,intSubRadByMass,whichlx=[14],figfilename='intsubradbymassLX14'+ext)

        SubRadSubmassFrac = SubhaloRadialSubmassFracPlugin()
        convergeplot(sheet,SubRadSubmassFrac,figfilename='subradsubmassfracLX'+ext,recalc=myrecalc)
        paper_stackplot(14,SubRadSubmassFrac,figfilename='stackLX14_subradsubmassfrac'+ext)

    #testhid = firsttwelve[2]
    #haloplot(testhid,12,pluglist=[Nvmax,SHMF,Profile,MassAccr],recalc=myrecalc)
    #haloplot(testhid,14,pluglist=[SubPhase],recalc=myrecalc)
    #haloplot(testhid,12,pluglist=[SubProfile],recalc=myrecalc)
    #haloplot(1130025,14,pluglist=[SubVelProfile])

    plt.close('all')
