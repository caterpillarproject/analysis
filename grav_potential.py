...
def fitTriaxialNFW(v,a,b,c,ahat,bhat,chat):
    snap = TM.getNsnaps(v)-1
    cat = TM.getRSCatalogue(snap,v)
    ...
    pos=TM.getPOS(snap,v,particles)
...
for hpath in halo_paths:
    ...
    ax1.text(-rvir*.8,rvir*.8, 'True Data $\sqrt{|\phi|}$ (km/s)',color='white',fontsize=14)
    
    im2=ax2.imshow(np.sqrt(smoothPhi), extent=extent,vmin=vmin,vmax=vmax,aspect='equal')
    ax2.set_xlabel('x [kpc]')
    ax2.set_ylim((-rvir,rvir))
    ax2.set_xlim((-rvir,rvir))
    ax2.text(-rvir*.8,rvir*.8, 'NFW Fit $\sqrt{|\phi|}$ (km/s)',color='white',fontsize=14)
    ...
    percent_diff = 100*(np.sqrt(matrix)-np.sqrt(smoothPhi))/np.sqrt(smoothPhi)
    ax3 = fig.add_axes([.67,.10,.28,.87])
    im3 = ax3.imshow(percent_diff,extent=extent,aspect='equal')
    ax3.set_xlabel('x [kpc]')
    ax3.set_ylim((-rvir,rvir))
    ax3.set_xlim((-rvir,rvir))
    ax3.text(-rvir*.8,rvir*.8, 'Residual (%)',color='black',fontsize=14)
