...
#img_path = '/bigbang/data/AnnaGroup/Dropbox/caterpillar_plots/zoom/halos/'
img_path = '/nfs/blank/h4231/bgriffen/'
...
halo_dirs = htils.get_paper_paths()
print halo_dirs

# get host halos
parenthalos = htils.load_pcatz0()
...
mvirhosts_all = np.array(hosts['vmax'])
...
mvirsubs_all = np.array(subs['vmax'])
...
#    f, axs = plt.subplots(6, 3, figsize=(12,14))
    f, axs = plt.subplots(2, 3, figsize=(22,14))
...
#    ax3 = axs[axi,2]
    ...
    mask_mass_hosts = (mvirhosts_all >= vmax_cut)
 ...
 #           placetext_direct(ax3,yposi,zposi,mstring,'normal',10)
    ...
    mask_mass_sub = (mvirsubs_all >= vmax_cut)
...
#    ax3.set_yticklabels([])
    ...
    for sub_diri in sub_dirs[-2:]:
        ictype, LX, NV = htils.get_zoom_params(sub_diri)
        zoomid = htils.load_zoomid(sub_diri)
...
#	print axi
 	...
 	contamination_radius = htils.get_quant_zoom(sub_diri,'min2')
        plotxyzprojr(ax1,ax2,ax3,np.array([0,0,0]).T,contamination_radius,axislabels=True,color='k',linestyle='--',linewidth=2)
        
        # load catalogues
        halodata = htils.load_rscat(sub_diri,255)

        allhalos = halodata.data
        hosts = halodata.get_hosts()
#       condhosts = (np.array(hosts['mvir'])/hubble >= mcut/10.)
        condhosts = (np.array(hosts['vmax']) > vmax_cut)
	...
	vmaxhost = float(allhalos.ix[zoomid]['vmax'])
	...
	vmaxhosts = hosts['vmax'][condhosts]
        ...
        mstring = '%3.0f' % (vmaxhost)

        placetext_direct(ax1,0.,0.,mstring,'bold',12)
        placetext_direct(ax2,0.,0.,mstring,'bold',12)
        placetext_direct(ax3,0.,0.,mstring,'bold',12)
        placetext(ax1,0.05,0.9,"LX: "+str(LX),'bold',12)

        for idi,x,y,z,rvir,mvir,vmax in zip(hostids,xposhosts,yposhosts,zposhosts,rvirhosts,mvirhosts,vmaxhosts):
            #if mvir > mcut and idi != haloid and mvir < mvirhost and \
            if vmax > vmax_cut and \
        ...
        condsubs = (np.array(subs['vmax']) >= vmax_cut)        
...
#        plotxyzprojr(ax1,ax2,ax3,np.array([0,0,0]).T,circut3,axislabels=False,color='c',linestyle='-',linewidth=2)
    ...
    #print "Writing to file:",img_path + 'H'+str(haloid)+'_XYZ_HALOS.png'
    print "Writing to file:",'H'+str(haloid)+'_XYZ_HALOS.png'
