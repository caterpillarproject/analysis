...
if __name__=="__main__":
    ...
    ## TBTF
    TBTF = TBTFPlugin()
    fig = convergeplot(sheet,TBTF,whichlx=[14],usehaloname=True)
    axlist = fig.axes
    for ax in axlist:
        ax.tick_params(which='major',axis='both', color='k', length=6,width=2)
        ax.tick_params(which='minor',axis='both', color='k', length=3,width=2)
    fig.savefig('brendan_TBTFLX14_s1.png',bbox_inches='tight')

    fig = convergeplot(1,TBTF,whichlx=[13],usehaloname=True)
    axlist = fig.axes
    for ax in axlist:
        ax.tick_params(which='major',axis='both', color='k', length=6,width=2)
        ax.tick_params(which='minor',axis='both', color='k', length=3,width=2)
    fig.savefig('brendan_TBTFLX13_s1.png',bbox_inches='tight')

    fig = convergeplot(2,TBTF,whichlx=[13],usehaloname=True)
    axlist = fig.axes
    for ax in axlist:
        ax.tick_params(which='major',axis='both', color='k', length=6,width=2)
        ax.tick_params(which='minor',axis='both', color='k', length=3,width=2)
    fig.savefig('brendan_TBTFLX13_s2.png',bbox_inches='tight')
