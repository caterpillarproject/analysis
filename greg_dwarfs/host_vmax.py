...
def vmaxfunction(vmaxes, histrange):
    hist, r_array = np.histogram(np.log10(vmaxes), bins=histrange)
    #print hist, 'histogram'
    x_array = getMidpoints(r_array)
    dV = 10.**r_array[1:]-10.**r_array[0:-1] #Mass size of bins in non-log space.
    dNdV = hist/dV
    return [x_array, dNdV]
...
def shvmax_function(cat, haloid, target_halo_vmax=151,factor=1):
    ...
    histrange = np.arange(min_vmax,max_vmax+.01,1./bins_per_dex)
    radius=factor*cat.ix[haloid]['rvir']
    vmaxes_sub = np.array(cat.get_subhalos_within_halo(haloid, radius)['vmax'])
    [x_array_sub, y_sub] = vmaxfunction(vmaxes_sub, histrange)
    dNdV = y_sub*(target_halo_vmax)/(cat.ix[haloid]['vmax'])
    return 10**x_array_sub, dNdV
...
def peak_vmax_function(hpath,cat,haloid,target_halo_vmax=151, version='peak',lx=14):
    ...
    histrange = np.arange(min_vmax,max_vmax+.01,1./bins_per_dex)
    [x_array_sub, y_sub] = vmaxfunction(peak_vmaxes, histrange)
    dNdV = y_sub*(target_halo_vmax)/(cat.ix[haloid]['vmax'])
    return 10**x_array_sub, dNdV
...
def plotFits(version='peak',lx=14,target_halo_vmax=151,factor=1):
    ...
    if version=='normal':
        ...
        plt.savefig('hostfigs/HostHaloVmax'+ext)

    if version=='peak':
        plt.figtext(.15,.25, 'Vmax definition at peak vmax')
        plt.savefig('hostfigs/HostHaloPeakVmax'+ext)
